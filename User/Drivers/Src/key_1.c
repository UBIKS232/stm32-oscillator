...
static key_hw_context_t key_1_hw = {
    .gpiox = key_1_GPIO_Port, .gpio_pin = key_1_Pin, .active_low = 1U};

static key_handle_t key_1_handle = {0};

static uint8_t key_1_read_pin(key_hw_context_t* hw_context) {
...
void key_1_init(void){
    key_init(&key_1_handle, key_1_read_pin, &key_1_hw);
    key_register_callback(&key_1_handle,key_1_callback);
    key_task_register(&key_1_handle);
}